...
(function(){
  ...
  // Token is stored in localStorage (not a cross-domain cookie) because the
  // frontend (Hostinger) and backend (Render) live on different domains, and
  // browsers increasingly block third-party/cross-site cookies.
  ...
  async function apiRequest(path, body){
    const res = await fetch(`${API_BASE}${path}`, {
      ...
      credentials: 'include', // still send/receive the cookie too, harmless for local dev
  ...
  if(registerForm){
    ...
    registerForm.addEventListener('submit', async (e) => {
      ...
      try{
        const data = await apiRequest('/auth/register', { name, email, password: pw });
        setToken(data.token);
